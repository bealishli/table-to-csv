...
function extractTables() {
  ...
  tableEls.forEach((table, idx) => {
    ...
    if (hasThead && !hasTbody) {
      ...
      let bodyTable = null;
      let bodyIdx = -1;
      // 在 DOM 中查找下一个兄弟 table
      let sibling = table.nextElementSibling;
      while (sibling) {
        if (sibling.tagName === 'TABLE') {
          const sibIdx = Array.from(tableEls).indexOf(sibling);
          if (sibIdx > idx && sibling.querySelector('tbody') && !sibling.querySelector('thead')) {
            bodyTable = sibling;
            bodyIdx = sibIdx;
            break;
          }
        }
        // 也检查子元素（可能 table 在一个容器 div 内）
        const nestedTable = sibling.querySelector('table');
        if (nestedTable && nestedTable.querySelector('tbody') && !nestedTable.querySelector('thead')) {
          const nestedIdx = Array.from(tableEls).indexOf(nestedTable);
          if (nestedIdx > idx) {
            bodyTable = nestedTable;
            bodyIdx = nestedIdx;
            break;
          }
        }
    ...
    // 情况2：只有 tbody 没有 thead 的表格（已被上面的逻辑作为 body 合并过）
    // 跳过已处理的
    if (!hasThead && hasTbody && !hasTh) {
      // 如果紧跟在一个 head-only table 后面，已被合并，跳过
      // 但如果独立存在，作为普通表格处理
      const prevTable = table.previousElementSibling;
      const prevIsHeadTable = prevTable?.tagName === 'TABLE' && prevTable.querySelector('thead') && !prevTable.querySelector('tbody');
      // 也检查父级
      const parentPrev = table.parentElement?.previousElementSibling;
      const parentPrevIsHeadTable = parentPrev?.querySelector?.('table') &&
        parentPrev.querySelector('table').querySelector('thead') &&
        !parentPrev.querySelector('table').querySelector('tbody');

      if (prevIsHeadTable || parentPrevIsHeadTable) {
        // 已被合并处理，跳过
        return;
      }
    }
  ...
  function addTableResult(result, idx, matrix, hasHeader) {
    ...
    if (hasHeader) {
      ...
      matrix.forEach((row, ri) => {
        // 如果这行不是全空，且前一行或自身是表头区域
        if (row.some(c => c !== '')) {
          // 简化：第一行非空行作为表头
          if (headerEndIdx === 0) headerEndIdx = 1;
        }
      });
      // 实际上前面已经解析过，hasHeader 时第一行就是表头
      headers = matrix[0] || [];
